...
function onlyEvenValues(arr) {
  const evenArray = []
  arr.forEach(e => (e % 2 == 0 ? evenArray.push(e) : false))
  return evenArray
}
...
function showFirstAndLast(arr) {
  ...
  arr.forEach(e => {
    firstAndLastArray.push(e[0].concat(e.slice(-1)[0]))
  })
...
function vowelCount(str) {
  ...
  splitArr.forEach(letter => {
    //make letter total case insensitive
    let lowerCasedLetter = letter.toLowerCase()
    //check if letter is a vowel (indexOf returns -1 if no index exists for a given value)
    if (vowels.indexOf(lowerCasedLetter) !== -1) {
      /*if the object already has a property with the vowel name, 
        then add 1 to the value of the property*/
      if (obj[lowerCasedLetter]) {
        obj[lowerCasedLetter]++
      } else {
        //otherwise, create a new property with property key set as the vowel and the value set to one
...
function valTimesIndex(arr) {
  const newValsTimesIndex = arr.map((e, idx) => {
    return e * idx
  })
  return newValsTimesIndex
}
...
function extractKey(arr, key) {
  const extractedKeys = arr.map(e => e[key])
...
Examples:
    ...
    e['Elie Schoppik', 'Tim Garcia', 'Matt Lane', 'Colt Steele']
*/

function extractFullName(arr) {
  const extractedFullName = arr.map(e => `${e['first']} ${e['last']}`)
...
function filterByValue(arr, key){
  const filteredValues =  arr.filter(e => e[key])
...
function find(arr, searchValue) {
    const found = arr.filter(e=>e===searchValue)
...
function findInObj(arr, key, searchValue) {
    const foundObj = arr.filter(e=>e[key]===searchValue)
    return foundObj[0]
}
...
function removeVowels(str) {
    ...
    const removedVowels = lowercaseStr.filter(letter=>!vowels.includes(letter))
...
function doubleOddNumbers(arr) {
    const doubledOddNums = arr.filter(e=>e%2===1).map(e=>e*2)
